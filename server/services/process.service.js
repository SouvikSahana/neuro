...
const processImage = async (imageFilename,user,email) => {
    try {
        if(!email){
            throw new Error("There is authentication failed")
        }
...
const processReady=async()=>{
    ...
    if (isProcessing) {
        console.log("Previous process is still running. Skipping this interval...");
        return;
      }
    ...
    try{
        ...
        const files=await collection.find({
            ...
            "metadata.type": { $in: ["prescription", "bill", "report"] },
          })
          .toArray();
          for(let i=0;i<files?.length;i++){
            console.log("Processing img: "+ files[i]?.filename)
            await processImage(files[i]?.filename,files[i]?.metadata?.user,files[i]?.metadata?.email)
...
setInterval(processReady, 600000);
